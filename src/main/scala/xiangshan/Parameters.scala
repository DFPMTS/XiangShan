...
(
  ...
  val intSchdParams = {
    ...
    SchdBlockParams(Seq(
      ...
      IssueBlockParams(Seq(
        ...
        ExeUnitParams("BJU2", Seq(BrhCfg, JmpCfg, I2fCfg, VSetRiWiCfg, VSetRiWvfCfg, I2vCfg), Seq(IntWB(port = 4, 0), VfWB(1, 0), V0WB(port = 1, 0), VlWB(port = intSchdVlWbPort, 0), FpWB(port = 2, 1)), Seq(Seq(IntRD(2, 1)), Seq(IntRD(3, 1)))),
      ), numEntries = IssueQueueSize, numEnq = 2, numComp = IssueQueueCompEntrySize),
      IssueBlockParams(Seq(
  ...
  val fpSchdParams = {
    ...
    SchdBlockParams(Seq(
      IssueBlockParams(Seq(
        ExeUnitParams("FEX0", Seq(FaluCfg, FcvtCfg, F2vCfg, FmacCfg), Seq(FpWB(port = 0, 0), IntWB(port = 0, 2), VfWB(port = 2, 0), V0WB(port = 2, 0)), Seq(Seq(FpRD(0, 0)), Seq(FpRD(1, 0)), Seq(FpRD(2, 0)))),
  ...
  val vfSchdParams = {
    ...
    SchdBlockParams(Seq(
      ...
      IssueBlockParams(Seq(
        ExeUnitParams("VFEX4", Seq(VfdivCfg, VidivCfg), Seq(VfWB(port = 0, 2), V0WB(port = 0, 2)), Seq(Seq(VfRD(0, 2)), Seq(VfRD(1, 2)), Seq(VfRD(2, 2)), Seq(V0RD(0, 2)), Seq(VlRD(0, 2)))),
  ...
  val memSchdParams = {
    ...
    SchdBlockParams(Seq(
      ...
      IssueBlockParams(Seq(
        ExeUnitParams("VLSU0", Seq(VlduCfg, VstuCfg, VseglduSeg, VsegstuCfg), Seq(VfWB(3, 0), V0WB(3, 0), VlWB(port = 2, 0)), Seq(Seq(VfRD(3, 0)), Seq(VfRD(4, 0)), Seq(VfRD(5, 0)), Seq(V0RD(1, 0)), Seq(VlRD(1, 0)))),
      ), numEntries = memIQSize, numEnq = 2, numComp = memCompSize),
      IssueBlockParams(Seq(
        ExeUnitParams("VLSU1", Seq(VlduCfg, VstuCfg), Seq(VfWB(4, 0), V0WB(4, 0), VlWB(port = 2, 0)), Seq(Seq(VfRD(6, 0)), Seq(VfRD(7, 0)), Seq(VfRD(8, 0)), Seq(V0RD(2, 0)), Seq(VlRD(2, 0)))),
      ), numEntries = memIQSize, numEnq = 2, numComp = memCompSize),
      IssueBlockParams(Seq(
