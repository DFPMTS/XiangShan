...
//    accessArray.io.read.last.bits.way_en := refillPipe.io.prefetch_flag_write.bits.way_en
  ...
  wb.io.miss_req_conflict_check(LoadPipelineWidth) := mainPipe.io.wbq_conflict_check
  mainPipe.io.wbq_block_miss_req   := wb.io.block_miss_req(LoadPipelineWidth)

  wb.io.miss_req_conflict_check(LoadPipelineWidth + 1).valid := missReqArb.io.out.valid
  wb.io.miss_req_conflict_check(LoadPipelineWidth + 1).bits  := missReqArb.io.out.bits.addr
  missQueue.io.wbq_block_miss_req := wb.io.block_miss_req(LoadPipelineWidth + 1)
